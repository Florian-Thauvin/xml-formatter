 ...
 */
export function processXmlString(
  rawXml: string,
  xmlOptions?: IXmlFormatting
): string {
...
function manageOneLineComment(line: string) {
  ...
  const endCommentIndex: number =
    line.indexOf(xmlCommentEndChar) + xmlCommentEndChar.length;
...
function addLine(line: string, optionnalSpacing: number = 0) {
  ...
  const indentation: string = options.indentation.repeat(
    actualIndentation + tempIndentation + optionnalSpacing
  );
...
function splitLine(line: string, indentation: string) {
  ...
  for (let j = 0; j < possibleSplittings.length; j++) {
    ...
    if (newLineWidth >= options.maxLineLenght - indentation.length) {
      ...
      const index =
        line.search(possibleSplittings[j - 1]) +
        possibleSplittings[j - 1].length;
      ...
      lineWidth =
        possibleSplittings[j - 1].length + possibleSplittings[j].length;
